...
function SortableCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="text-xs text-neutral-600 mb-1 block">Subtítulo</label>
        <input
          type="text"
          value={card.subtitle}
...
export default function NosotrosEditor({ proposal, onUpdate }: Props) {
  const nosotros = proposal.nosotros;

  function updateTitle(field: "titleLine1" | "titleHighlight" | "titleLine2", value: string) {
    onUpdate({ nosotros: { ...nosotros, [field]: value } });
  }
  ...
  function addCard() {
    onUpdate({
      nosotros: {
        ...nosotros,
        cards: [...nosotros.cards, { image: "", icon: "👤", title: "Nuevo miembro", subtitle: "Rol", description: "Descripción" }],
      },
    });
  }
  ...
  function removeCard(index: number) {
    onUpdate({
      nosotros: { ...nosotros, cards: nosotros.cards.filter((_, i) => i !== index) },
    });
  }

  function updateCard(index: number, field: keyof CardItem, value: string) {
    const updated = [...nosotros.cards];
    updated[index] = { ...updated[index], [field]: value };
    onUpdate({ nosotros: { ...nosotros, cards: updated } });
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const reordered = arrayMove(nosotros.cards, oldIndex, newIndex);
    onUpdate({ nosotros: { ...nosotros, cards: reordered } });
  }
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <input
          ...
          placeholder="CONOCE AL"
        />
        <input
          type="text"
          value={nosotros.titleHighlight}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-medium text-neutral-500 uppercase tracking-wider">
            Miembros ({nosotros.cards.length})
          </h3>
        </div>

        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={nosotros.cards.map((_, i) => `card-${i}`)} strategy={verticalListSortingStrategy}>
            <div className="space-y-4">
              {nosotros.cards.map((card, index) => (
                <SortableCard
                  key={index}
                  card={card}
                  index={index}
